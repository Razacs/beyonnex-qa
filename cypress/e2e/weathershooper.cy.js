  ...
  it("should click Buy moisturizers or Buy sunscreens button based on temperature", () => {
    ...
    cy.get("#temperature")
      ...
      .then((temperature) => {
        ...
        if (temp < 19) { // If temperature is less than 19, buy moisturizers
          cy.contains("Buy moisturizers").click();
          cy.url()
            .should("include", "/moisturizer")
            .then(() => { //adding moisturizers
        ...
        } else if (temp > 34) { // If temperature is greater than 34, buy sunscreens
        ...
        } else { // If temperature is between 19 and 34, throw an error
    ...
    cy.get(".table-striped tbody tr td:nth-child(2)").then(($prices) => {
      ...
      // Click on "Pay with Card" and fill payment details
    cy.contains("Pay with Card")
      ...
      .then(() => {
        ...
        cy.intercept("POST", "https://api.stripe.com/v1/tokens").as( // Intercept the request to the Stripe API for token generation
          "stripeRequest"
        );
